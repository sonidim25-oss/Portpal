...
export type TauriFixture = {
  ports: FixturePort[];
  events: FixtureEvent[];
  traffic: Record<number, Array<{ connections: number; timestamp: number }>>;
};
    ...
    Object.defineProperty(window, '__TAURI_EVENT_PLUGIN_INTERNALS__', {
      value: {
        unregisterListener: (_event: string, eventId: number) => {
          const callbackId = listenerCallbacks.get(eventId);
    ...
    Object.defineProperty(window, '__TAURI_INTERNALS__', {
      value: {
        ...
        async invoke(cmd: string, args?: unknown) {
          ...
          switch (cmd) {
            case 'get_ports':
              return structuredClone(data.ports);
            case 'get_port_events':
              return structuredClone(data.events);
            case 'get_port_traffic':
              return structuredClone(data.traffic);
            case 'restart_process': {
              ...
              if (keys.join(',') !== 'pid,port')
                throw new Error(`restart_process rejected unexpected arguments: ${keys.join(',')}`);
              return undefined;
            }
            case 'kill_process':
            ...
            case 'plugin:event|listen': {
              ...
              const handler = (args as { handler?: number } | undefined)?.handler;
              if (handler !== undefined) listenerCallbacks.set(eventId, handler);
              return eventId;
            }
            default:
