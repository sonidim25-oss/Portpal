...
describe('usePortPalData', () => {
  ...
  it('reports a missing scan tool instead of an empty port list', async () => {
    const gateway = createGateway({
      getPorts: vi.fn().mockRejectedValue({
        code: 'tool_missing',
        tool: 'lsof',
  ...
  it('marks already-listed ports stale when background scanning degrades', async () => {
    ...
    const gateway = createGateway({
      onScanDegraded: vi.fn().mockImplementation(async (handler) => {
        degrade = handler;
        return () => {};
      }),
    });
    ...
    act(() =>
      degrade({ code: 'tool_failed', tool: 'lsof', message: '`lsof` failed: permission denied' }),
    );
  ...
  it('clears the degraded state and rescans once scanning recovers', async () => {
    ...
    const gateway = createGateway({
      ...
      onScanRecovered: vi.fn().mockImplementation(async (handler) => {
        recover = handler;
        return () => {};
      }),
    });
  ...
  it('surfaces typed backend policy errors without removing the process', async () => {
    const gateway = createGateway({
      killProcess: vi.fn().mockRejectedValue({
        code: 'not_observed',
        message: 'PID is not a currently observed listening process',
      }),
    });
    const { result } = renderHook(() => usePortPalData(gateway));
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      expect(await result.current.killPort(port)).toBe('failed');
    });
  ...
  it('blocks protected listeners even when another row for the PID appears safe', async () => {
    const gateway = createGateway({
      getPorts: vi
        .fn()
        .mockResolvedValue([port, { ...port, port: 5432, process_name: 'Postgres' }]),
    });
    const { result } = renderHook(() => usePortPalData(gateway));
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      expect(await result.current.killPort(port)).toBe('critical');
    });
  ...
  it('atomically skips duplicate in-flight kills of the same PID', async () => {
    let finish!: () => void;
    const gateway = createGateway({
      killProcess: vi.fn().mockImplementation(
        () =>
  ...
  it('restarts by port and pid only, never forwarding a command or path', async () => {
    ...
    expect(JSON.stringify(vi.mocked(gateway.restartProcess).mock.calls)).not.toContain(
      'npm run dev',
    );
  });
  ...
  it('does not restart a port with no recorded command or path, and says why', async () => {
    ...
    await act(async () =>
      result.current.restartPort({ ...port, start_cmd: null, project_path: null }),
    );
  ...
  it('falls back to the process name when a refused restart has no project name', async () => {
    ...
    await act(async () =>
      result.current.restartPort({ ...port, project_name: null, start_cmd: null }),
    );
  ...
  it('keeps one toast slot: a second failure replaces the first and reuses the timer', async () => {
    ...
    await act(async () => {
      await result.current.killPort(port);
    });
    await act(async () => {
    ...
    await act(async () => {
      await result.current.killPort({ ...port, pid: 4321 });
    });
  ...
  it('reports a readable restart failure instead of a stringified object', async () => {
    const gateway = createGateway({
      restartProcess: vi
        .fn()
        .mockRejectedValue({ code: 'spawn_failed', message: 'working directory is gone' }),
    });
    const { result } = renderHook(() => usePortPalData(gateway));
    await waitFor(() => expect(result.current.loading).toBe(false));

    await act(async () => {
      await result.current.restartPort(port);
    });
