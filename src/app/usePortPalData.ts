...
function observedStarts(
  ...
  return events.reduce<Record<number, number>>(
    (observedAt, event) => {
...
export function usePortPalData(
  gateway: PortPalGateway = tauriPortPalGateway,
): UsePortPalDataResult {
  ...
  useEffect(() => {
    ...
    subscribe(
      gateway.onPortsUpdated((updatedPorts) => {
      ...
      (unlisten) => {
        unlistenPorts = unlisten;
      },
      'port updates',
    );

    subscribe(
      gateway.onPortEvents((updatedEvents) => {
    ...
    subscribe(
      gateway.onScanDegraded((scanError) => {
        setLoading(false);
        setErrors((current) => ({ ...current, ports: scanError.message }));
      }),
      (unlisten) => {
        unlistenDegraded = unlisten;
      },
      'scan health',
    );

    subscribe(
      gateway.onScanRecovered(() => {
        setErrors((current) => ({ ...current, ports: null }));
        void refreshPorts();
      }),
      (unlisten) => {
        unlistenRecovered = unlisten;
      },
      'scan recovery',
    );
    ...
    subscribe(
      gateway.onScanCompleted(() => {
        setLastScanAt(Date.now());
      }),
      (unlisten) => {
        unlistenScanCompleted = unlisten;
      },
      'scan heartbeat',
    );
  ...
  const killPort = useCallback(
    async (port: PortInfo): Promise<KillOutcome> => {
