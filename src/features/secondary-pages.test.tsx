...
const traffic: TrafficByPort = {
  3000: [
    { connections: 2, timestamp: 1 },
    { connections: 5, timestamp: 2 },
  ],
  3001: [
    { connections: 3, timestamp: 1 },
    { connections: 1, timestamp: 2 },
  ],
...
describe('secondary pages', () => {
  ...
  it('never merges a folder named like a system service with the real service', () => {
    ...
    render(
      <ServicesPage ports={tricky} traffic={{}} loading={false} error={null} onRetry={vi.fn()} />,
    );
  ...
  it('preserves log event rows and refresh while retaining stale events on error', async () => {
    ...
    render(
      <LogsPage events={events} loading={false} error="events unavailable" onRefresh={onRefresh} />,
    );
  ...
  it('separates still-loading from confirmed-empty on the derived pages', () => {
    const services = render(
      <ServicesPage ports={[]} traffic={{}} loading error={null} onRetry={vi.fn()} />,
    );
    ...
    const traffic = render(
      <TrafficPage ports={[]} traffic={{}} loading error={null} onRetry={vi.fn()} />,
    );
