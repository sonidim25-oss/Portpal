...
export function PortsPage({
  ...
  const protectedListener = (port: PortInfo) =>
    [port, ...latest.current.ports.filter((p) => p.pid === port.pid)].find(isCriticalProcess);
  ...
  const confirmKills = async () => {
    ...
    try {
      for (const port of targets) {
        ...
        if (latest.current.killing.has(port.pid)) {
          busy++;
          continue;
        }
        ...
        if (!latest.current.ports.some((p) => samePort(p, port))) {
          failed++;
          continue;
        }
        try {
      ...
      if (targets.length > 1) {
        setKillSummary(
          `Killed ${killed}, failed ${failed}, skipped critical ${critical}, skipped busy ${busy}`,
        );
      }
    } finally {
  ...
  const selectedLivePort = selected
    ? filteredPorts.find((port) => samePort(port, selected))
    : undefined;
  ...
  const toggleSelection = (port: PortInfo) => {
    setSelected((current) =>
      current && samePort(port, current) ? null : { port: port.port, pid: port.pid },
    );
  };
    ...
    <section className="ports-page" aria-labelledby="ports-page-title">
      ...
      <div className="ports-page__controls">
        ...
        <Button
          ...
          onClick={() => {
            setKillSummary(null);
            setKillSelection(uniqueProcesses(filteredPorts));
          }}
        ...
        <div className="ports-page__table-pane">
          {error ? (
            ...
            <EmptyState
              ...
              description="Start a server and it will appear here."
            />
          ) : (
            <PortTable
              ports={filteredPorts}
      ...
      {killSummary && (
        <div className="kill-summary" role="status">
          {killSummary}
        </div>
      )}
      {killSelection &&
        (() => {
          ...
          const siblings = ports.filter(
            (p) =>
              ...
              !selectedKeys.has(`${p.pid}:${p.port}`) &&
              filteredKeys.has(`${p.pid}:${p.port}`),
          );
          const hidden = ports.filter(
            (p) =>
              selectedPids.has(p.pid) &&
              !selectedKeys.has(`${p.pid}:${p.port}`) &&
              !filteredKeys.has(`${p.pid}:${p.port}`),
          ...
          return (
            <KillConfirmation
              count={killSelection.length}
              protectedPorts={killSelection.flatMap((port) => {
                const protectedPort = protectedListener(port);
                return protectedPort ? [protectedPort] : [];
              })}
              hiddenPorts={hidden}
              siblingPorts={siblings}
              onCancel={() => setKillSelection(null)}
              onConfirm={() => void confirmKills()}
            />
          );
        })()}
    </section>
